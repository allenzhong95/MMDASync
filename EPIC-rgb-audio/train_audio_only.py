...
if __name__ == '__main__':
    ...
    audio_cls_model.load_state_dict(
    ...
    log_path = base_path + "log%s2%s_audio.csv" % (
    ...
    optim = torch.optim.Adam(list(audio_cls_model.parameters()),
                             lr=lr, weight_decay=1e-4)
    scheduler = lr_scheduler.StepLR(optim, step_size=60, gamma=0.1)
    train_dataset = EPICDOMAINAudio(split='train', domain=args.source_domain,
                                    modality='rgb', )
    ...
    validate_dataset = EPICDOMAINAudio(split='test', domain=args.target_domain,
                                       modality='rgb',)
    validate_dataloader = torch.utils.data.DataLoader(
        validate_dataset, batch_size=batch_size, num_workers=5, shuffle=True,
        pin_memory=(device.type == "cpu"), drop_last=True)
                ...
                with tqdm.tqdm(total=len(dataloaders[split])) as pbar:
                    for (i, (spectrogram, labels)) in \
                        ...
                        pbar.set_postfix_str(
                            "Average loss: {:.4f}, Current loss: {:.4f},\
                            Accuracy: {:.4f}".format(
                                total_loss / float(count), loss.item(),
                                acc / float(count)))
                        pbar.update()
                    f.write("{},{},{},{}\n".format(
                        epoch_i, split, total_loss / float(count),
                        acc / float(count)))
                    f.flush()
                    # if epoch_i < 1:
            scheduler.step()
