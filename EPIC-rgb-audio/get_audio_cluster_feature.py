...
for _, line in test_file.iterrows():
    ...
    feature_wise_data[int(labels)].append(
...
for ii in range(8):
    for file_id, image in enumerate(feature_wise_data[ii]):
        ...
        feat1 = np.load(args.source_domain + '_sound_features/' + video_id +
                        "_%010d.npy" % start_index)
...
for label_id in range(8):
    ...
    for cluster_num in range(1, 21):
        kmeans = KMeans(n_clusters=cluster_num, init='k-means++', max_iter=300,
    ...
    kmeans = KMeans(n_clusters=kn.knee, init='k-means++', max_iter=300,
                    n_init=10, random_state=0)
    kmeans.fit(feature1)
