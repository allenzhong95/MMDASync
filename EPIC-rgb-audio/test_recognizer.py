...
config_file = 'configs/recognition/slowfast/' + \
    'slowfast_r101_8x8x1_256e_kinetics400_rgb2.py'
...
checkpoint = torch.load(f"checkpoints/best_{args.source_domain}2{args.target_domain}_audio.pt")
...
checkpoint = torch.load("checkpoints/best_%s2%s_slow_flow_transformer_cls.pt"
                        % (args.source_domain, args.target_domain,))
...
test_file = pd.read_pickle(
    glob_para.pkl_path + args.target_domain + "_test.pkl")
...
acc = 0
save_path = 'preds/'+args.source_domain + '2' + args.target_domain + \
    '_transformer4cls/'
...
for i, sample1 in enumerate(data1):
    ...
    with torch.no_grad():
        ...
        x_slow, x_fast = model.module.backbone.get_predict(adapted_v_feat)
        # batch_size*5, 2048, 8, 7, 7
        ...
        predict1, _, _ = recognizer(visual_feat=v_feat.detach(),
